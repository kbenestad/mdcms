...
| `mdcms completion [bash\|zsh\|fish]` | Print the one-off commands that switch on Tab completion for that shell (defaults to the shell in `$SHELL`). Completion covers command names, options, and registered site names — the `NAME` argument of `view`/`build`/`update`/`config`/`bundle`/`fetch-deps`/`delete` completes from the registry via `complete_site_names()`. The completion script itself is generated by click (`_MDCMS_COMPLETE=<shell>_source mdcms`); this command only prints the setup. It attaches to the installed `mdcms` command, not to `python3 mdcms.py`, because click derives the env-var name from the program name. |
