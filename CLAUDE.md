...
| `mdcms update [name]` | Update a site's renderer: overwrites `index.html` with the version this CLI ships (preserving the site's `<title>`) and bumps the `CURRENT VERSION` marker in `config.yml`. Accepts `--path`; `--force` re-downloads even if already current. Site content (pages, posts, nav.yml, theme.yml) is untouched. |
