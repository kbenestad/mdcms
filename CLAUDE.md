...
| `mdcms delete <name> [name...]` | Remove one or more sites from the registry. Does not delete files. Prompts for confirmation unless `-y`/`--yes` is passed. |
...
| `mdcms upgrade` | Upgrade the `mdcms` CLI itself to the latest release. Detects pip, pipx, dpkg-managed `.deb`, or standalone-binary installs and upgrades accordingly (binary installs are downloaded and swapped in place; a dpkg-managed install downloads the new `.deb` and installs it via `dpkg -i`, re-execing under `sudo` if needed, so the package database stays in sync). `--force` reinstalls even if already current. |
| `mdcms completion [bash\|zsh\|fish]` | Set up Tab completion for that shell (defaults to the shell in `$SHELL`). Rarely needed by hand: `_autoinstall_completion()`, called from the `cli` group callback, does it on the first run after install — it writes the script click generates (`get_completion_class`) to `~/.config/mdcms/mdcms-complete.<shell>` and appends one source line to `~/.bashrc`/`~/.zshrc`; fish gets `~/.config/fish/completions/mdcms.fish` and no startup-file edit. The script file is the marker: once it exists the auto-install does nothing, so deleting it (and the source line) opts out for good, as does `MDCMS_NO_COMPLETION=1`. Skipped when stdout is not a TTY, so CI/CD is untouched. Completion covers command names, options, and registered site names — the `NAME` argument of `view`/`build`/`update`/`config`/`bundle`/`fetch-deps`/`delete` completes from the registry via `complete_site_names()`. It attaches to the installed `mdcms` command, not to `python3 mdcms.py`, because click derives the env-var name from the program name. |
...
**`app/mdcms.json` is regenerated on every release.** It is the manifest `mdcms register` downloads the starter template by — read straight off `main` — so it must list exactly the files present in `app/`. Being a checked-in static list, it goes stale the moment a template file is added or removed, and a stale entry 404s mid-download and aborts the whole `register` (this is what the trimmed icon pack did before v0.8.3: the manifest still listed the six `LEGACY_ICONS` and was missing the four panel icons). The `publish` job's "Refresh app/ template manifest" step now regenerates it from the actual `app/` tree and commits it alongside the version bump, so no one has to remember. If you change `app/`'s file list outside a release, regenerate by hand: `python3 -c "import pathlib, mdcms; mdcms.generate_site_manifest(pathlib.Path('app'))"`.
...
**`mdcms build` keeps a site's `assets/icons/` trimmed to what's actually referenced.** `CORE_ICONS` (in both `mdcms.py` and `index.html` — keep the two lists in sync) are rendered unconditionally (theme toggle, hamburger, search, pagination arrows, panel toggles, callout `info`/`warning`/`success`/`error`) and are always kept. `OPTIONAL_ICONS` — the alternate `nav-section-expand-icon`/`nav-section-collapse-icon` choices plus `language` — are only downloaded into `assets/icons/` when a site's `config.yml`/`theme.yml` actually names one (`categories-selecticon`, `nav-section-expand-icon`, `nav-section-collapse-icon`, or a `theme.yml` callout `icon:` override); otherwise `mdcms build` removes them if present. Icon files with a name outside `CORE_ICONS`/`OPTIONAL_ICONS`/`LEGACY_ICONS` are left alone — treated as custom, never auto-downloaded or deleted. `LEGACY_ICONS` are icons dropped from the stock pack entirely (unreferenced anywhere in `index.html`, `config.yml`, or any theme) — never downloaded, only cleaned up if a site still has a leftover copy.
