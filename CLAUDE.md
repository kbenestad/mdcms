...
**`mdcms build` keeps a site's `assets/icons/` trimmed to what's actually referenced.** `CORE_ICONS` (in both `mdcms.py` and `index.html` — keep the two lists in sync) are rendered unconditionally (theme toggle, hamburger, search, pagination arrows, panel toggles, callout `info`/`warning`/`success`/`error`, code-fence copy button `content_copy`/`check`) and are always kept. `OPTIONAL_ICONS` — the alternate `nav-section-expand-icon`/`nav-section-collapse-icon` choices plus `language` — are only downloaded into `assets/icons/` when a site's `config.yml`/`theme.yml` actually names one (`categories-selecticon`, `nav-section-expand-icon`, `nav-section-collapse-icon`, or a `theme.yml` callout `icon:` override); otherwise `mdcms build` removes them if present. Icon files with a name outside `CORE_ICONS`/`OPTIONAL_ICONS`/`LEGACY_ICONS` are left alone — treated as custom, never auto-downloaded or deleted. `LEGACY_ICONS` are icons dropped from the stock pack entirely (unreferenced anywhere in `index.html`, `config.yml`, or any theme) — never downloaded, only cleaned up if a site still has a leftover copy.
